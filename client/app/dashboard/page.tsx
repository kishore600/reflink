      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl shadow-sm">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-black">Completed Sessions</p>
              <p className="text-2xl font-bold text-gray-900">{user?.completedSessions || 0}</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {referrals.filter(r => ['submitted', 'under-review', 'interview'].includes(r.status)).length}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href="/dashboard/sessions" className="text-blue-600 hover:text-blue-700 text-sm font-medium">
              View all
            </Link>
          </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-black">
                        with {session.jobSeeker._id === user?._id ? session.employee.name : session.jobSeeker.name}
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
                        {new Date(session.scheduledAt).toLocaleDateString()} • {session.duration} mins
                      </p>
                    </div>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      session.status === 'scheduled' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'
                    }`}>
                      {session.status}
                    </span>
                  </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href="/dashboard/referrals" className="text-blue-600 hover:text-blue-700 text-sm font-medium">
              View all
            </Link>
          </div>
                ...
                <div key={referral._id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-black">
                        {referral.employee._id === user?._id ? 'You referred' : 'Referred by'} {referral.employee.name}
                      </p>
                    </div>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      referral.status === 'submitted' ? 'bg-blue-100 text-blue-800' :
                      referral.status === 'interview' ? 'bg-green-100 text-green-800' :
                      referral.status === 'hired' ? 'bg-purple-100 text-purple-800' :
      ...
      <div className="mt-8 bg-white rounded-2xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <p className="text-sm text-black">Find developers to collaborate with</p>
          </Link>
          ...
          >
            ...
            <p className="text-sm text-black">View and edit your public profile</p>
          </Link>
          ...
          >
            <div className="text-2xl mb-2">⚡</div>
            <h3 className="font-semibold text-gray-900">Add Offers</h3>
            <p className="text-sm text-black">Create new consulting offers</p>
          </Link>
        </div>
      </div>
    </div>
